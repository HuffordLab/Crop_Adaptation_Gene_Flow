...
%\gmj{Are there newer resequencing methods to discuss here?  DART-Seq, etc?}
%\lwang{I think it is fine to mention two of these methods, it is not our focus.}
...
Additionally, $RND_{min}$ accounts for variable mutation rate, which is detected based on branch length to an outgroup:  $RND_{min} = d_{min}/d_{out}$, where $d_{min}$ is the minimum sequence distance between haplotypes in species X and Y and $d_{out}$ equals $(d_{XO} + d_{YO})/2$, the average sequence distance between each species and the outgroup ($O$).
...
%\lwang{These are more complex statistics to infer the overall demography, like Isolate with migration, which we have defined in the beginning that we will not touch it in the manuscript}
...
%\gmj{This paragraph about application seems like it might not belong in this section.  Maybe it should be a part of the introduction?  Or, maybe each of these examples could be mentioned directly following the method(s) that was used in the study?}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=17.35cm]{./FigsAndTables/Figure1}
	\caption{Hapmix vectors indicating introgression level from \emph{mexicana} to Mexican highland maize on chromosome 4. Data adopted from Hufford et al. \cite{hufford2013genomic}. ELP: EL Porvenir; IXT: Ixtlan ; NAB: Nabogame; OPO: Opopeo; PUR: Puruandiro; SAC: Santa Clara; SAP: San Pedro; TEN: Tenango del Aire; XOC: Xochimilco.}
...
Despite many conditions favorable to their prevalence, to date, individual examples of adaptive alleles naturally introgressing into domesticated Asian rice are not clear (though \cite{zhao2010genomic} emphasizes the necessity of research on this topic).
Contemporary populations of wild rices show evidence of this history of local adaptation.
Wild \emph{rufipogon} and \emph{nivara} both maintain high genotypic and phenotypic diversity and exhibit locally-adaptive traits (\emph{rufipogon} is adapted to forested wetland environments, whereas \emph{nivara} is adapted to dryer conditions and has life cycle adaptations to survive grazing pressure), suggesting that either offered (and offers) adaptive genetic potential.
Domesticated varieties have likewise adapted to local environments.
The \emph{japonicas} are generally more adapted to upland temperate regions (and are split into subgroups adapted to more temperate or more tropical conditions), the \emph{indicas} are best suited for tropical lowland environments, \emph{aus} matures quickly and tolerates drought stress, and two deepwater varieties (\emph{rayada} and \emph{ashwina}) are said to be selected for the environment along the Ganges river \cite{vaughan2008evolving, garris2005genetic}.
...
However, as with many other domesticated crops, gene flow between wild and domesticated rices is highly asymmetric (estimates of wild rice admixture in domesticated rice are less than 5 percent \cite{wang2017asian}).
This asymmetry is due in part to the closed floret architecture of the domesticated rice, which hinders outcrossing.
Still, barriers to crop-to-wild introgression may have been less severe during early domestication as the inbreeding reproductive system of domesticated rice would not have been as firmly established \cite{vaughan2008evolving}.
Furthermore, the contemporary distribution of wild rice does not capture the full range and diversity of wild rice during early domestication and range expansion of Asian rice.
...
\gmj{I had saved these notes from an earlier version of the paper.  There is plenty of evidence of hybridization and introgression (both farmer-mediated and and natural), and some adaptive traits have been conferred, but I could not find direct evidence of specific cases of adaptive natural introgression from wild relatives to cultivated potatoes. This new paper fills that gap, if we decide that the paper's merits are substantial enough that we can overlook its flaws.}
\gmj{If we decide to keep and use the new potato introgression publication, that would be the natural choice for the potato reference, whether or not we include the write-up of the potato case study.}

Modern potato (\emph{Solanum tuberosum}) is believed to have been domesticated in southern Peru in sympatry with a multitude of wild relatives about 6000 BP, although the exact location and formal classification and phylogenetic relationships between these taxa have long been disputed \cite{huaman2002reclassification, spooner2005single, pickersgill1977origins, hawkes1988evolution}.
...
>>>>>>> 70a60e4d8de5497bac8f9ee07dccbe37f50bc8dd
\\\toprule  
\rowcolor{white}
...
Introgression between a crop and newly-encountered taxa decreases divergence from these donors.
...
%This is a revolutionary departure from the simpler model of a temporally-constrained domestication event with subsequent dispersal, and as such, offers new questions and new opportunities for investigation.
...
%Understanding the histories of domestication and adaptation inform and empower efforts to develop and improve crop systems.
...
%As a species, domesticated crops are among our first and greatest inventions.
