...
%Using such methods as the D-statistic or HAPMIX to identify introgressant haplotypes and measure their length, for a given rate of linkage disequilibrium breakdown due to outcrossing frequency, one can estimate the recency of an introgression event.

The recency of an introgression event can be assessed by measuring patterns of linkage disequilibrium (LD) and identity by state (IBS) between the domesticated and wild populations.
Recent introgression will result in high LD \cite{chakraborty1988recombination} and genomic regions of high IBS \cite{gusev2011architecture}, both of which break down over time.
LD will break down faster in crops with high outcrossing frequencies due to greater recombination rates.
LD breakdown can be assessed with D-statistics or HAPMIX, and decrease in IBS can be assessed with with several tools, including the PLINK toolset \cite{purcell2007plink}.
Determining that a crop genome has low levels of LD or IBS with wild relatives can be cited as evidence that any introgressions are not recent (as in Poets et al. 2015 \cite{Poets2015).
